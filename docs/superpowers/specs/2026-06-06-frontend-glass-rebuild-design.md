...
- **实时数据可视化 Hero(玻璃面板):** `LIVE ●` 脉冲 + 问候 + 实时时钟;**OEE 径向仪表**、KPI 行(WIP/良率/产量 + 升降 delta)、**本班产出区域图**。数据来源见 §6。
...
- **安全合规条(代「信任徽章」):** 细玻璃条:SSO 安全登录 · RBAC 角色权限 · 审计日志 · 99.98% 可用率 · ISO 9001(展示性,内部应用语境)。
...
## 6. Hero 数据可视化 — 数据来源(需用户确认)

后端**当前不提供实时制造指标**(OEE/WIP/良率),仅有链接目录 + 用户/枚举/标签数据。为既保住出彩的 Hero 又不虚构「实时」数据:

- **默认方案(本期,无后端改动):** Hero 分两层 ——
  1. **真实门户数据可视化:** 系统总数、在线/维护/停用 状态分布、按类别计数、最近/常用(本地)。这些来自 `/api/portal/home`,是**真实**的。
  2. **制造 KPI 图表(OEE/WIP/良率/产出)= 经可插拔 `MetricsProvider` 适配器**:本期注入**明确标注「示例 / SAMPLE」**的占位数据(不冒充真实)。生产接真实指标端点 = **未来后端任务**(不在本次前端范围)。
- 备选:若希望立即真实,需后端新增 metrics 端点(独立后端项目)。
