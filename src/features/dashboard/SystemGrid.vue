...
const emit = defineEmits<{ 'access-request': [HomeLink]; 'favorite-changed': [{ id: number; favorite: boolean }] }>()
  ...
  <div class="space-y-6">
    <section v-for="c in categories" :key="c.categoryCode">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
        <SystemCard v-for="l in c.links" :key="l.id" :link="l" @blocked="onBlocked" @access-request="emit('access-request', $event)" @favorite-changed="emit('favorite-changed', $event)" />
      </div>
    </section>
